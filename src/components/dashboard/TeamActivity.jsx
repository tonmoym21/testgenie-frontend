  ...
  return (
    <div className="bg-white rounded-xl border border-surface-200 p-5">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="font-semibold text-sm text-surface-900">Recent Activity</h3>
          <p className="text-xs text-surface-400">Latest test operations</p>
        </div>
      </div>
          ...
          return (
            <div key={activity.id} className="flex gap-3">
              ...
              <div className="flex flex-col items-center">
                <div className="w-8 h-8 rounded-full bg-surface-100 flex items-center justify-center text-xs font-semibold text-surface-600 shrink-0">
                  {activity.avatar}
                </div>
              ...
              <div className={`flex-1 min-w-0 ${isLast ? 'pb-0' : 'pb-4'}`}>
                <div className="flex items-baseline gap-1.5 flex-wrap">
                  <span className="text-sm font-medium text-surface-900">{activity.user}</span>
                  <span className={`text-sm ${config.color}`}>{config.verb}</span>
                  <span className="text-sm text-surface-600 truncate">{activity.target}</span>
                </div>
