        ...
        <div className="flex border-b border-surface-200">
          {tabs.map((tab) => (
            <button
              ...
              className={`px-4 py-2.5 text-xs font-medium transition-colors border-b-2 -mb-px ${activeTab === tab.id ? 'border-brand-500 text-brand-600' : 'border-transparent text-surface-400 hover:text-surface-600'}`}
            >
              {tab.label}
              {tab.count > 0 && <span className="ml-1.5 bg-surface-100 text-surface-500 rounded-full px-1.5 text-[10px]">{tab.count}</span>}
            </button>
          ))}
        </div>
            ...
            <div>
              {raw.body ? (
                <div className="relative">
                  ...
                  <pre className="bg-surface-50 rounded-lg p-4 text-xs font-mono overflow-auto max-h-96 text-surface-700 leading-relaxed">
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="border-b border-surface-200">
                        <th className="text-left px-3 py-2 font-medium text-surface-400 uppercase w-1/3">Key</th>
                        <th className="text-left px-3 py-2 font-medium text-surface-400 uppercase">Value</th>
                      </tr>
                    </thead>
                    <tbody>
                      {Object.entries(raw.headers).map(([key, value]) => (
                        <tr key={key} className="border-b border-surface-100 last:border-0 hover:bg-surface-100/50">
                          <td className="px-3 py-2 font-mono font-medium text-surface-700">{key}</td>
          ...
          {activeTab === 'assertions' && (
            <div>
              {assertions.length > 0 ? (
                  ...
                  {assertions.map((a, i) => (
                    <div key={i} className={`flex items-start gap-3 p-3 rounded-lg ${a.passed ? 'bg-green-50' : 'bg-red-50'}`}>
                      ...
                      <div className="text-xs">
                        <span className="font-medium text-surface-700">{a.target}{a.path ? '.' + a.path : ''}</span>
                        <span className="text-surface-400 mx-1">{a.operator}</span>
                        <span className="font-mono text-surface-600">{JSON.stringify(a.expected)}</span>
                        {!a.passed && <p className="text-red-600 mt-1">{a.message}</p>}
                      </div>
                    </div>
                  ))}
                  <div className="flex gap-4 pt-2 text-xs text-surface-500">
                ...
                <p className="text-sm text-surface-400 text-center py-8">No assertions</p>
              )}
            </div>
          )}
          ...
          {activeTab === 'logs' && (
            <div className="max-h-80 overflow-y-auto">
              {logs.filter((l) => l.level !== 'debug').map((log, i) => (
                <div key={i} className="flex items-start gap-2 text-xs font-mono py-1 border-b border-surface-50 last:border-0">
                  ...
                  <span className={log.level === 'error' ? 'text-red-600' : 'text-surface-600'}>{log.message}</span>
                </div>
              ))}
            </div>
          )}
