...
const STATUS_BADGE = { draft: 'bg-gray-100 text-gray-600', ready: 'bg-green-100 text-green-700', archived: 'bg-yellow-100 text-yellow-700' };
const RUN_STATUS_BADGE = { passed: 'bg-green-100 text-green-700', failed: 'bg-red-100 text-red-700', running: 'bg-blue-100 text-blue-700', queued: 'bg-gray-100 text-gray-600' };
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      ...
      {bulkSummary && selected.size > 0 && (
        <div className="mb-5 p-3 rounded-lg border border-gray-200 bg-gray-50 flex items-center gap-4 text-xs">
          <span className="font-medium text-gray-600">Readiness Summary:</span>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="divide-y divide-gray-100">
            {assets.map((asset) => (
              <div key={asset.id} className="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition-colors">
                <input type="checkbox" checked={selected.has(asset.id)} onChange={() => toggleSelect(asset.id)} className="rounded border-gray-300" />
                <div className="flex-1 min-w-0">
                  <Link to={`/projects/${projectId}/automation/${asset.id}`} className="font-medium text-sm text-gray-900 hover:text-brand-600 truncate block">
                    {asset.name}
                  </Link>
                ...
                <div className="w-28 flex flex-wrap gap-1">
                  ...
                  {(asset.categories || []).length > 2 && <span className="text-[10px] text-gray-400">+{asset.categories.length - 2}</span>}
                </div>
                <div className="w-20">
          ...
          {pagination.total > 20 && (
            <div className="flex justify-center gap-2 mt-6">
              ...
              <span className="text-sm text-gray-400 py-2">Page {pagination.page} of {Math.ceil(pagination.total / 20)}</span>
