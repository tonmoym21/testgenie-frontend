...
const STATUS_STYLES = {
  draft:    'bg-surface-100 text-surface-600 ring-1 ring-inset ring-surface-300/40',
  active:   'bg-brand-50 text-brand-700 ring-1 ring-inset ring-brand-600/15',
  passed:   'bg-emerald-50 text-emerald-700 ring-1 ring-inset ring-emerald-600/15',
  failed:   'bg-red-50 text-red-700 ring-1 ring-inset ring-red-600/15',
  archived: 'bg-surface-100 text-surface-500 ring-1 ring-inset ring-surface-300/40',
};
...
export default function ProjectDetailPage() {
  ...
  if (!loading && !project) {
    return (
      <div className="page">
        <div className="empty">
          <h3 className="text-lg font-semibold text-surface-800 mb-1">Project not found</h3>
          <Link to="/projects" className="btn-secondary mt-4"><ArrowLeft size={14} /> Back to projects</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="page">
      <Link to="/projects" className="inline-flex items-center gap-1.5 text-sm text-surface-500 hover:text-surface-800 mb-4 transition-colors">
      ...
      {showAnalyze && (
        <div className="card p-6 mb-6 ring-1 ring-brand-200 bg-gradient-to-br from-brand-50/60 to-purple-50/40">
              ...
              <button key={value} onClick={() => setAnalysisType(value)}
                className={`text-left p-3 rounded-xl border transition-all ${
                  analysisType === value
                    ...
                    : 'border-surface-200/60 bg-white/60 hover:bg-white hover:border-surface-300'
                }`}>
          ...
          {analysisError && (
            <div className="mt-4 bg-red-50 text-red-700 text-sm px-4 py-3 rounded-lg border border-red-200">{analysisError}</div>
          )}
          ...
          {analysisResult && (
            <div className="mt-4 bg-white rounded-xl ring-1 ring-surface-200/60 p-5">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-xs text-surface-400">{analysisResult.tokenUsage?.total} tokens</span>
              </div>

              {analysisResult.result?.summary && (
                <p className="text-sm text-surface-700 mb-4 leading-relaxed">{analysisResult.result.summary}</p>
              )}
                ...
                <div className="space-y-2">
                  ...
                  {analysisResult.result.gaps.map((gap, i) => (
                    <div key={i} className="bg-purple-50 rounded-lg p-3 ring-1 ring-inset ring-purple-100">
                      ...
                      <p className="text-sm text-surface-600 mt-1">{gap.description}</p>
                    </div>
                  ))}
                </div>
              )}
              ...
              {analysisResult.result?.recommendations?.length > 0 && (
                <div className="mt-4">
                  <h5 className="text-[10px] font-semibold uppercase tracking-wider text-surface-400 mb-2">Recommendations</h5>
                  <ul className="space-y-1">
                    {analysisResult.result.recommendations.map((rec, i) => (
                      <li key={i} className="text-sm text-surface-600 flex gap-2">
                        <span className="text-brand-500 mt-0.5">&#8226;</span>{rec}
                      </li>
                    ))}
                  </ul>
                </div>
              ...
              {analysisResult.result?.risks?.length > 0 && (
                <div className="space-y-2 mt-4">
                  <h5 className="text-[10px] font-semibold uppercase tracking-wider text-surface-400">Risks</h5>
                  {analysisResult.result.risks.map((risk, i) => (
                    <div key={i} className="bg-red-50 rounded-lg p-3 ring-1 ring-inset ring-red-100">
                      <div className="flex items-center gap-2">
                        <Shield size={14} className="text-red-500" />
                        <span className="text-sm font-medium text-surface-900">{risk.area}</span>
                        <span className="badge bg-red-100 text-red-700">{risk.riskLevel}</span>
                      </div>
                      <p className="text-sm text-surface-600 mt-1">{risk.recommendation}</p>
                    </div>
                  ))}
                </div>
              )}
              ...
              {analysisResult.result?.duplicates?.length > 0 && (
                <div className="space-y-2 mt-4">
                  <h5 className="text-[10px] font-semibold uppercase tracking-wider text-surface-400">Duplicates</h5>
                  {analysisResult.result.duplicates.map((dup, i) => (
                    <div key={i} className="bg-amber-50 rounded-lg p-3 ring-1 ring-inset ring-amber-100">
                      <p className="text-sm font-medium text-surface-900">{dup.reason}</p>
                      <p className="text-sm text-surface-600 mt-1">{dup.mergeRecommendation}</p>
                    </div>
                  ))}
                </div>
              )}
              ...
              {analysisResult.result?.issues?.length > 0 && (
                <div className="space-y-2 mt-4">
                  <h5 className="text-[10px] font-semibold uppercase tracking-wider text-surface-400">Issues</h5>
                  {analysisResult.result.issues.map((issue, i) => (
                    <div key={i} className="bg-blue-50 rounded-lg p-3 ring-1 ring-inset ring-blue-100">
                      <span className="text-sm font-medium text-surface-900">{issue.issue}</span>
                      <p className="text-sm text-surface-600 mt-1">{issue.suggestion}</p>
                    </div>
                  ))}
                </div>
              )}
        ...
        <div className="fixed inset-0 bg-surface-950/50 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in" onClick={() => setShowCreate(false)}>
          <div className="card p-6 w-full max-w-lg shadow-soft-lg animate-slide-up max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-start justify-between mb-4">
              <h2 className="text-lg font-semibold text-surface-900">Add test case</h2>
            ...
            <form onSubmit={handleCreateTestCase} className="space-y-4">
              ...
              <div>
                <label className="label">Test steps / content</label>
                ...
                <div className="flex gap-2 flex-wrap">
                  {['low', 'medium', 'high', 'critical'].map((p) => (
                    <button key={p} type="button" onClick={() => setNewPriority(p)}
                      className={`badge capitalize cursor-pointer transition-colors ${newPriority === p ? PRIORITY_STYLES[p] : 'bg-surface-50 text-surface-400 ring-1 ring-inset ring-surface-200'}`}>
                      {p}
                    </button>
                  ))}
                </div>
              ...
              <div className="flex gap-3 justify-end pt-2">
                ...
                <button type="submit" disabled={creating} className="btn-primary">
                  {creating && <Loader2 size={16} className="animate-spin" />} Add test case
                </button>
              </div>
            </form>
          </div>
                ...
                <div className="flex gap-2 flex-wrap">
                  {['low', 'medium', 'high', 'critical'].map((p) => (
                    <button key={p} type="button" onClick={() => setEditPriority(p)}
                      className={`badge capitalize cursor-pointer transition-colors ${editPriority === p ? PRIORITY_STYLES[p] : 'bg-surface-50 text-surface-400 ring-1 ring-inset ring-surface-200'}`}>
                      {p}
                    </button>
                  ))}
                </div>
                ...
                <div className="flex gap-2 flex-wrap">
                  {Object.keys(STATUS_STYLES).map((s) => (
                    <button key={s} type="button" onClick={() => setEditStatus(s)}
                      className={`badge capitalize cursor-pointer transition-colors ${editStatus === s ? STATUS_STYLES[s] : 'bg-surface-50 text-surface-400 ring-1 ring-inset ring-surface-200'}`}>
                      {s}
                    </button>
                  ))}
                </div>
      ...
      {jiraLinkTc && (
        <div className="fixed inset-0 bg-surface-950/50 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in" onClick={() => setJiraLinkTc(null)}>
          <div className="card p-6 w-full max-w-md shadow-soft-lg animate-slide-up" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-start justify-between mb-4">
              <div>
                <h2 className="text-lg font-semibold text-surface-900">Link to Jira issue</h2>
                <p className="text-sm text-surface-500 mt-0.5">Linking: <span className="font-medium text-surface-700">{jiraLinkTc.title}</span></p>
              </div>
            ...
            {jiraSearching && <p className="text-sm text-surface-400 mb-2">Searching...</p>}
            {jiraResults.length > 0 && (
              <div className="rounded-lg ring-1 ring-surface-200/60 divide-y divide-surface-100 max-h-48 overflow-y-auto mb-3">
                {jiraResults.map((issue) => (
                  <button key={issue.key} onClick={() => handleJiraLink(issue)} disabled={jiraLinking}
                    ...
                    <span className="text-sm text-surface-700 truncate">{issue.summary}</span>
                    <ExternalLink size={12} className="text-surface-400 shrink-0 ml-auto" />
                  </button>
                ))}
              </div>
            )}
      ...
      {!loading && testCases.length === 0 && (
        <div className="empty">
          ...
          <h3 className="text-lg font-semibold text-surface-800 mb-1">No test cases yet</h3>
          <p className="text-surface-500 text-sm mb-6 max-w-xs">Add your first test case to start building coverage for this project.</p>
          <button onClick={() => setShowCreate(true)} className="btn-primary"><Plus size={16} /> Add test case</button>
        </div>
      )}
      ...
      {!loading && testCases.length > 0 && (
        <div className="space-y-2">
          {testCases.map((tc) => (
            <div key={tc.id}
              className={`card p-4 transition-all ${selectedIds.includes(tc.id) ? 'ring-2 ring-brand-400/40 border-brand-300 bg-brand-50/30' : ''}`}>
                ...
                <div className="flex items-start gap-3 flex-1 min-w-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <h3 className="font-medium text-sm text-surface-900">{tc.title}</h3>
                    ...
                    <p className="text-xs text-surface-500 mt-1.5 whitespace-pre-wrap font-mono leading-relaxed">{tc.content}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-1 shrink-0">
                  {tc.jiraIssueKey ? (
                    ...
                    <button onClick={() => { setJiraLinkTc(tc); setJiraSearch(''); setJiraResults([]); setJiraError(''); }}
                      className="icon-btn hover:text-blue-500" aria-label="Link to Jira">
                      <Link2 size={14} />
                    </button>
                  )}
                  <button onClick={() => openEdit(tc)} className="icon-btn" aria-label="Edit"><Pencil size={14} /></button>
                  <button onClick={() => handleDeleteTestCase(tc.id)} className="icon-btn hover:text-red-500" aria-label="Delete"><Trash2 size={14} /></button>
                </div>
              </div>
            </div>
          ))}
