      ...
      <div className="p-4 max-h-80 overflow-auto">
          ...
          raw.headers && Object.keys(raw.headers).length > 0 ? (
            <div className="bg-gray-50 rounded-lg overflow-hidden">
              <table className="w-full text-xs">
                ...
                <tbody>{Object.entries(raw.headers).map(([k, v]) => (<tr key={k} className="border-b border-gray-100 last:border-0 hover:bg-gray-100/50"><td className="px-3 py-2 font-mono font-medium text-gray-700">{k}</td><td className="px-3 py-2 font-mono text-gray-500 break-all">{v}</td></tr>))}</tbody>
              </table>
            </div>
        ...
        {activeTab === 'chain' && (
          Object.keys(extracted).length > 0 ? (
            <div>
              <p className="text-xs text-gray-500 mb-3">These values are available as <code className="bg-gray-100 px-1 rounded">{'{{response.prev.NAME}}'}</code> in the next test.</p>
              <div className="bg-gray-50 rounded-lg overflow-hidden">
                <table className="w-full text-xs">
                  <thead><tr className="border-b border-gray-200"><th className="text-left px-3 py-2 font-medium text-gray-400 uppercase">Name</th><th className="text-left px-3 py-2 font-medium text-gray-400 uppercase">Value</th></tr></thead>
                  <tbody>{Object.entries(extracted).map(([k, v]) => (<tr key={k} className="border-b border-gray-100 last:border-0"><td className="px-3 py-2 font-mono text-purple-700">{k}</td><td className="px-3 py-2 font-mono text-gray-600">{v}</td></tr>))}</tbody>
                </table>
              </div>
            </div>
          ) : <p className="text-sm text-gray-400 text-center py-8">No extractors defined — add extractors in the test config to chain responses</p>
        )}
        {activeTab === 'logs' && (
          logs.length > 0 ? (
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-2">
              <input type="checkbox" id="isPublic" checked={isPublic} onChange={(e) => setIsPublic(e.target.checked)} className="rounded border-gray-300 text-brand-600" />
              <label htmlFor="isPublic" className="text-sm text-gray-600">Make link public (anyone with the link)</label>
            </div>
          ...
          {/* Existing shares */}
          {loading ? <div className="text-center py-4"><Loader2 size={20} className="animate-spin mx-auto text-gray-400" /></div> : (
              ...
              {shares.length === 0 ? (
                ...
                <div key={s.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg text-sm">
                  <div className="min-w-0 flex-1">
                    <span className={`text-xs font-medium px-2 py-0.5 rounded mr-2 ${s.permission === 'fork' ? 'bg-purple-100 text-purple-700' : s.permission === 'run' ? 'bg-blue-100 text-blue-700' : 'bg-gray-200 text-gray-600'}`}>{s.permission}</span>
                    <span className="text-gray-500 text-xs truncate">{s.sharedWithEmail || (s.isPublic ? 'Public link' : 'Private link')}</span>
                  </div>
                  <div className="flex items-center gap-1.5 shrink-0 ml-2">
                    <button onClick={() => copyLink(s.shareToken)} className="p-1.5 text-gray-400 hover:text-purple-600 rounded">
                      {copied === s.shareToken ? <Check size={14} className="text-green-500" /> : <Copy size={14} />}
                    </button>
                    <button onClick={() => handleRevoke(s.id)} className="p-1.5 text-gray-400 hover:text-red-500 rounded"><Trash2 size={14} /></button>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="flex-1 overflow-auto p-5 space-y-4">
          ...
          <div className="card p-4">
            ...
            <div className="flex gap-1 border-b border-gray-200 mb-4">
              {tabs.map((tab) => (
                <button key={tab} onClick={() => setActiveTab(tab)} className={`px-3 py-2 text-xs font-medium capitalize border-b-2 -mb-px ${activeTab === tab ? 'border-brand-500 text-brand-600' : 'border-transparent text-gray-400 hover:text-gray-600'}`}>
            ...
            {activeTab === 'auth' && (
              <div>
                ...
                {authType === 'bearer' && <div className="bg-gray-50 rounded-lg p-4"><label className="text-xs text-gray-500 mb-1 block">Token</label><input value={bearerToken} onChange={(e) => setBearerToken(e.target.value)} className="input py-1.5 text-sm font-mono" placeholder="eyJhbG… or {{env:API_TOKEN}}" /></div>}
                {authType === 'api_key' && (
                  <div className="bg-gray-50 rounded-lg p-4 space-y-3">
                    ...
                    <div><label className="text-xs text-gray-500 mb-1 block">Key Value</label><input value={apiKeyValue} onChange={(e) => setApiKeyValue(e.target.value)} className="input py-1.5 text-sm font-mono" placeholder="{{env:API_KEY}}" type="password" /></div>
                    <div className="flex gap-2">
                      <button type="button" onClick={() => setApiKeyLocation('header')} className={`px-3 py-1.5 rounded text-xs font-medium ${apiKeyLocation === 'header' ? 'bg-brand-100 text-brand-700' : 'bg-gray-100 text-gray-500'}`}>Header</button>
                      <button type="button" onClick={() => setApiKeyLocation('query')} className={`px-3 py-1.5 rounded text-xs font-medium ${apiKeyLocation === 'query' ? 'bg-brand-100 text-brand-700' : 'bg-gray-100 text-gray-500'}`}>Query Param</button>
                    </div>
                  </div>
                )}
                {authType === 'basic' && (
            ...
            {activeTab === 'body' && (
              ['GET', 'DELETE'].includes(apiMethod)
                ? <p className="text-xs text-gray-400 text-center py-6">{apiMethod} requests don&apos;t have a body</p>
                : <div>
                    <div className="flex items-center justify-between mb-1">
                      <label className="text-xs text-gray-500">JSON Body</label>
            ...
            {activeTab === 'chain' && (
              <div>
                <div className="flex items-center justify-between mb-2">
                  <div>
                    <span className="text-xs font-medium text-gray-700">Response Extractors</span>
                    <p className="text-xs text-gray-400 mt-0.5">Extract values from this response for use as <code className="bg-gray-100 px-1 rounded">{'{{response.prev.NAME}}'}</code> in the next test</p>
                  </div>
                ...
                {extractors.length === 0 ? (
                  <div className="text-center py-6 bg-gray-50 rounded-lg">
                    ...
                    <p className="text-xs text-gray-400">No extractors — add one to chain this response into the next test</p>
                    <p className="text-xs text-gray-300 mt-1">Example: name=<code>userId</code>, path=<code>data.id</code> → use as <code>{'{{response.prev.userId}}'}</code></p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    {extractors.map((e, i) => (
                      <div key={i} className="flex gap-2 items-center bg-purple-50/50 rounded-lg p-2">
                        ...
                        <button onClick={() => removeExtractor(i)} className="p-1.5 text-gray-300 hover:text-red-500"><Trash2 size={14} /></button>
                      </div>
                    ))}
                  </div>
                )}
